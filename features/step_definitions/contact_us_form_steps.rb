...
Then /^a contact message record should be created$/ do
  ...
  ContactMessage.first.message.should eq("Five Tool Development is the firm for me.")
end

Then /^I should see an alert with the text "(.*?)"$/ do |arg1|
  page.has_selector?('.alert', text: "Thanks for contacting us. We will be in touch shortly.")
end